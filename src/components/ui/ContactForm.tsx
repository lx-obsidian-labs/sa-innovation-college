...
const COURSE_OPTIONS = [
  ...
  { category: "Education & Training", courses: ["Facilitator Course NQF 5", "Assessor Course", "Training & Development Practitioner NQF 5", "Learning & Development Practitioner NQF 6", "Work Based Learning Practitioner NQF 5", "Educare / Junior Teacher N4-N6", "Adult Literacy Teacher"] },
    ...
    <>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setShowConfirm(false)}>
          <div className="max-w-md w-full rounded-2xl bg-white p-6 shadow-2xl animate-scaleIn" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleSubmit} className="flex-1 px-4 py-2.5 text-sm font-semibold rounded-lg bg-[var(--color-primary)] text-white hover:bg-[var(--color-primary-dark)] transition-all">Send</button>
            </div>
          </div>
        </div>
      )}
      ...
      <form onSubmit={(e) => { e.preventDefault(); setShowConfirm(true); }} className="space-y-5" noValidate>
        <div className="grid sm:grid-cols-2 gap-5">
          <div>
            <label htmlFor="cf-name" className="block text-sm font-medium text-[var(--color-text-primary)] mb-1.5">Full Name <span className="text-red-500">*</span></label>
          ...
          <div>
            <label htmlFor="cf-phone" className="block text-sm font-medium text-[var(--color-text-primary)] mb-1.5">Phone Number <span className="text-red-500">*</span></label>
        ...
        <div>
          <label htmlFor="cf-email" className="block text-sm font-medium text-[var(--color-text-primary)] mb-1.5">Email Address <span className="text-red-500">*</span></label>
        ...
        <div>
          <label htmlFor="cf-course" className="block text-sm font-medium text-[var(--color-text-primary)] mb-1.5">Course Interested In</label>
        ...
        <div>
          <label htmlFor="cf-message" className="block text-sm font-medium text-[var(--color-text-primary)] mb-1.5">Your Message <span className="text-red-500">*</span></label>
          <textarea id="cf-message" rows={5} maxLength={2000} required value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} placeholder="Tell us more about what you're looking for..." className={cn(inputClass("message"), "resize-y")} />
        ...
        <button type="submit" disabled={state === "loading"} className={cn(
          "inline-flex items-center justify-center gap-2 w-full px-6 py-3 text-sm font-semibold uppercase tracking-wide rounded-lg transition-all duration-200",
          state === "loading"
            ? "bg-[var(--color-primary)]/60 text-white cursor-not-allowed"
            : "bg-[var(--color-primary)] text-white hover:bg-[var(--color-primary-dark)] hover:scale-[1.02] active:scale-[0.98] shadow-sm"
        )}>
          {state === "loading" ? (
            <>
